...
def tag(documents: list[str], lab_configuration):
    ...
    model = SentenceTransformer("sentence-transformers/all-MiniLM-L6-v2")
    tmp["Embedding"] = tmp["Translation"].applymap(
        lambda x: cp.array(model.encode(x).astype(float))
    )
    ...
    for col_name, model_name in text_classification_models:
        ...
        tmp[col_name] = tmp["Translation"].applymap(
            lambda x: [(y["label"], float(y["score"])) for y in pipe(x)[0]]
        )
    ...
    tokenizer = AutoTokenizer.from_pretrained("bert-large-uncased")
    tmp["Embedded"] = tmp["Translation"].applymap(
        lambda x: cp.array(
            tokenizer.encode_plus(
                x,
                add_special_tokens=True,
    ...
    tmp["Sentiment"] = tmp["Translation"].applymap(compounded_sentiment)
    ...
    tmp["FinancialSentiment"] = tmp["Translation"].applymap(compounded_financial_sentiment)
