...
def tag(documents: list[str], lab_configuration):
    ...
    # Create an empty cuDF DataFrame
    tmp = cudf.DataFrame()
    ...
    tmp["Embedding"] = tmp["Translation"].apply_map(
    ...
    for col_name, model_name in text_classification_models:
        ...
        tmp[col_name] = tmp["Translation"].apply_map(
    ...
    tmp["Embedded"] = tmp["Translation"].apply_map(
        lambda x: cp.array(
    ...
    # sentiment apply compounded_sentiment
    tmp["Sentiment"] = tmp["Translation"].apply_map(compounded_sentiment)
    
    # financial sentiment apply compounded_financial_sentiment
    tmp["FinancialSentiment"] = tmp["Translation"].apply_map(compounded_financial_sentiment)

    del tmp["Embedded"]
    tmp = tmp.to_pandas().to_dict(orient="records")
